    ...
    def perform_sweep(self, m, callback=None, graphic_callback=None):
        ...
        while True:
            ...
            if sys_label == "l" and 2 * sys_block.length == L:
                ...
                self.rbi = rbi
                return
    def output_after_finite_sweep(self):
        output = {}
        output['psi0'] = self.psi0
        output['rbi'] = self.rbi # restricted block indices
        output['sys_block'] = self.sys_block
        output['sys_trmat'] = self.sys_trmat 
        output['block_disk'] = self.block_disk
        output['trmat_disk'] = self.trmat_disk
        return output
...
def canonicalize(meas_desc):
    # This performs a stable sort on operators by site (so we are assuming
    # that operatprs on different sites commute). By doing this, we will
    # be able to combine operators ASAP as a block grows
...
class Yay(object):
    def __init__(self, m):
        ...
        self.refcnt = 1
#
def get_measurements_by_site_and_(measurements, model):
    measurements_by_site = {}
    processed_measurements = []
    for meas_desc in measurements:
        site_indices = set()
        ...
        for site_index in site_indices:
            measurements_by_site.setdefault(site_index, []).append(measurement)
    assert len(measurements) == len(processed_measurements)
    return measurements_by_site, processed_measurements
#
def get_site_class(model,L):
    LEFT_BLOCK, LEFT_SITE, RIGHT_BLOCK, RIGHT_SITE = 0, 1, 2, 3
    if model.boundary_condition == open_bc:
        sites_by_area = (
            range(0, L // 2 -1), # left block
    ...
    # from the above info, make a lookup of what class each site is in so
    # we can look it up easily.
    #
    site_class = [None] * L
    for i, heh in enumerate(sites_by_area):
...
if __name__ == "__main__":
    ...
    for m in m_sweep_list:
        print("Performing sweep with m =", m)
        dmrg.perform_sweep(m, callback=default_callback, 
                            graphic_callback=default_graphic_callback)
    #
    print("------perform measurement--------")
    #
    finite_sweep_output = dmrg.output_after_finite_sweep()
    psi0 = finite_sweep_output['psi0']
    rbi = finite_sweep_output['rbi']
    sys_block = finite_sweep_output['sys_block']
    sys_trmat = finite_sweep_output['sys_trmat']
    block_disk = finite_sweep_output['block_disk']
    trmat_disk = finite_sweep_output['trmat_disk']
    #
    print("-----after finite dmrg sweep------")
    print("psi0.shape:", psi0.shape)
    print("rbi.shape:", len(rbi)) # restricted basis indicies
    print("----information of the sys_block and sys_trmat of the last step of fdmrg----")
    print("length of sys_block:", sys_block[0])
    print("basis_size of sys_block:", sys_block[1])
    print("sys_trmat.shape:", sys_trmat.shape)
    print("----information of block_disk----")
    for index in list(block_disk.keys()):
        print("index:", index, "basis_size:", block_disk[index][1])
    print("----information of trmat_disk----")
    for index in list(trmat_disk.keys()):
        print("index:", index, "shape of trmat", trmat_disk[index].shape)
    #
    print("----------perform measurement------------")
    site_class = get_site_class(model=model, L=L)
    print("site_class:", site_class)
    measurements_by_site, processed_measurements = get_measurements_by_site_and_(
        model=model,measurements=measurements)
    for index in list(measurements_by_site.keys()):
        print("index:", index)
    #
    for it in processed_measurements:
        print(it)