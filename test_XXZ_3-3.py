    ...
    def perform_measurements(self, measurements):
        ...
        def handle_operators_on_site(site_index, area, some_dict, msize):
            for measurement in measurements_by_site[site_index]:
                ...
                for i, obj in enumerate(measurement):
                    ...
                    obj.built = True
                    print("first replace each operator on this site by an actual operator")
                    print("obj.obj.site_indices[0]:", obj.obj.site_indices[0])
                    print("obj.obj.operator_names[0]:", obj.obj.operator_names[0])
                    print("sso:", sso)
                    print("mat:", mat)
                    print("some_dict[obj.obj]:", some_dict[obj.obj])
                ...
                for i in range(len(measurement) - 1)[::-1]:
                    print("second, combine all operators that are possible to combine")
                    print("len(measurement)-1:", len(measurement)-1)
                    print("i:", i)
                    print("measurement[i].built:", measurement[i].built)
                    print("measurement[i + 1].built:", measurement[i + 1].built)
                    print("measurement[i].obj.area:", measurement[i].obj.area)
        ...
        def build_block(area, block_label):
            ...
            for i, site_index in enumerate(sites_by_area[area]):
                ...
                print("area:", area, "some_dict:", some_dict, "msize:", msize)
...
if __name__ == "__main__":
    ...
    def run_sample_spin_chain(boundary_condition, L=12):
        ...
        perform_dmrg(model, L=L, m_warmup=10, m_sweep_list=[20], 
                    target_sector=0, measurements=measurements)
