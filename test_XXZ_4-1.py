...
class DMRG(object):
    ...
    def perform_measurements(self, measurements):
            ...
            for measurement in measurements_by_site[site_index]:
                ...
                for i, obj in enumerate(measurement):
                    ...
                    try:
                        ...
                        mat = kron(identity(msize), sso) # 从左到右一直直乘到 sso 所在的格点位置。
                        print("从左到右一直直乘到 sso 所在是格点位置")
                        print("mat = kron(identity(msize), sso)")
                        print("mat.shape:", mat.shape)
                        print("结束单位矩阵和 sso 的直乘")
        ...
        def build_block(area, block_label):
            ...
            msize = 1
            for it in list(some_dict.keys()):
                print(it)
            for i, site_index in enumerate(sites_by_area[area]):
                # kronecker all the operators on the block
                print("+++++++++++++++++++++++++++++++++++++++++++++++++")
                print("kronecker all the operators on the block")
                print("sites_by_area:", sites_by_area)
                print("sites_by_area[area]:", sites_by_area[area])
                for k, v in some_dict.items():
                    v.m = kron(v.m, identity(model.d))
                    print("k:", k)
                    print("v.m = kron(v.m, identity(model.d))")
                ...
                if i == 0:
                    ...
                    print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
                    print("transform all the operators on the block")
                    print("----extract the trmat------------")
                    trmat = self.trmat_disk[block_label, i + 1]
                    print("block_label:", block_label, "(i + 1):", i + 1)
                    print("trmat.shape", trmat.shape)
                    print("some_dict.items():", some_dict.items())
                    for k, v in some_dict.items():
                        assert v.refcnt > 0
                        assert k.area == area
                        print("before truncation, v.m.shape:", v.m.shape)
                        v.m = rotate_and_truncate(v.m, trmat)
                        print("k:", k)
                        print("v.m = rotate_and_truncate(v.m, trmat)")
                        print("after truncation, v.m.shape:", v.m.shape)
                    print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
                    msize = trmat.shape[1]
                print("msize:", msize)    
            return msize
        ...
        print("*****************************************************")
        print("(5): 构造块算符结束，开始进行测量" )
        print("*****************************************************")
        print("mm_orig = [identity(lb_msize),identity(model.d),identity(rb_msize),identity(model.d),]")
        print("lb_msize:", lb_msize, "model.d:", model.d, "rb_msize:", rb_msize, "model.d:", model.d)
        print("information of LEFT_BLOCK in g_some_dict")
        for it in list(g_some_dict[0].keys()):   
            print("index of g_some_dict[0]:", it)
        print("information of LEFT_SITE in g_some_dict")
        for it in list(g_some_dict[1].keys()):   
            print("index of g_some_dict[1]:", it)
        print("information of RIGHT_BLOCK in g_some_dict")
        for it in list(g_some_dict[2].keys()):   
            print("index of g_some_dict[2]:", it)
        print("information of RIGHT_SITE in g_some_dict")
        for it in list(g_some_dict[3].keys()):   
            print("index of g_some_dict[3]:", it)    
        #
        ...
        for pm in processed_measurements:
            ...
            for obj in pm:
                ...
                print("mm[obj.obj.area] = g_some_dict[obj.obj.area][obj.obj].m")
                print("obj.obj.area:", obj.obj.area)
...
if __name__ == "__main__":
    ...
    # 考虑只测量单格点 Sz 的期望
    def run_sample_spin_chain(boundary_condition, L=12):
        ...
        measurements = ([[(i, "Sz")] for i in range(L)]) 
        perform_dmrg(model, L=L, m_warmup=10, m_sweep_list=[14], target_sector=0, measurements=measurements)
